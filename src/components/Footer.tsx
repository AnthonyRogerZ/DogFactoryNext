...
export default function Footer() {
  return (
    <footer className="bg-[#D3CFCF] py-4">
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between">
          {/* Contact Info */}
          <div className="text-xs md:text-sm">
            <a href="tel:0658166105" className="block hover:text-brand transition-colors">
              📞 06 58 16 61 05
            </a>
            <a 
            ...
            >
              🏠 79 rue de la Baste,{' '}
              <span className="hidden md:inline">77000 Vaux-le-Pénil</span>
              <span className="md:hidden block">77000 Vaux-le-Pénil</span>
            </a>
            <a 
              href="mailto:contact@dogfactory.fr" 
              className="block hover:text-brand transition-colors"
            >
              ✉️ contact@dogfactory.fr
            </a>
          </div>
        ...
        {/* Bottom Section */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-2 text-xs text-gray-600 mt-4 pt-3 border-t border-gray-400/20">
          ...
          <p className="text-[10px] md:text-xs"> {new Date().getFullYear()} Dog'Factory. Tous droits réservés.</p>
        </div>
      </div>
    </footer>
  );
